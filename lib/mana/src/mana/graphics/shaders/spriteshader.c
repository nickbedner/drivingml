...
u8 sprite_shader_init(struct SpriteShader* sprite_shader, struct APICommon* api_common, u32 width, u32 height, u8 supersample_scale, struct GBufferCommon* gbuffer_common, b8 depth_test, b8 depth_write, const uint_fast32_t msaa_samples, uint_fast32_t descriptors) {
  ...
  shader_settings->color_attachments = SPRITE_SHADER_COLOR_ATTACHEMENTS;
  shader_settings->vertex_attributes = SPRITE_SHADER_VERTEX_ATTRIBUTES;
...
// Note: This is a hack to get the render pass to the shader for vulkan
  ...
  shader_init(&(sprite_shader->shader), api_common, width, height, supersample_scale);
