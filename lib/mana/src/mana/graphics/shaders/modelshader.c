...
uint_fast8_t model_shader_init(struct ModelShader* model_shader, struct APICommon* api_common, uint32_t width, uint32_t height, uint_fast8_t supersample_scale, struct GBufferCommon* gbuffer_common, bool depth_test, bool depth_write, const uint_fast32_t msaa_samples, uint_fast32_t descriptors) {
  ...
  shader_settings->uniform_constant_state[2] = (struct ShaderAttributeSettings){.shader_stage = SHADER_STAGE_VERTEX_AND_FRAGMENT, .shader_position = 7};
...
// Note: This is a hack to get the render pass to the shader for vulkan
  ...
  shader_init(&(model_shader->shader), api_common, width, height, supersample_scale);

  return 0;
}

void model_shader_delete(struct ModelShader* model_shader, struct APICommon* api_common) {
  shader_delete(&model_shader->shader, api_common);

  vkDestroyDescriptorPool(api_common->vulkan_api.device, model_shader->shader.shader_common.shader_vulkan.descriptor_pool, NULL);
}
