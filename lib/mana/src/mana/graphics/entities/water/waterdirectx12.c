...
uint_fast8_t water_directx_12_init(struct WaterCommon* water_common, struct APICommon* api_common, struct Shader* shader, struct Texture* texture) {
  ...
  directx_12_graphics_utils_setup_constant_buffer(&(api_common->directx_12_api), sizeof(struct SpriteUniformBufferObject), &water_common->water_directx12.vertex_constant_buffer);
...
void water_directx_12_delete(struct WaterCommon* water_common, struct APICommon* api_common) {
  /*if (water_common->water_directx12.vertex_buffer) water_common->water_directx12.vertex_buffer->lpVtbl->Release(water_common->water_directx12.vertex_buffer), water_common->water_directx12.vertex_buffer = NULL;
  if (water_common->water_directx12.index_buffer) water_common->water_directx12.index_buffer->lpVtbl->Release(water_common->water_directx12.index_buffer), water_common->water_directx12.index_buffer = NULL;
  if (water_common->water_directx12.vertex_constant_buffer) water_common->water_directx12.vertex_constant_buffer->lpVtbl->Release(water_common->water_directx12.vertex_constant_buffer), water_common->water_directx12.vertex_constant_buffer = NULL;
  if (water_common->water_directx12.fragment_constant_buffer) water_common->water_directx12.fragment_constant_buffer->lpVtbl->Release(water_common->water_directx12.fragment_constant_buffer), water_common->water_directx12.fragment_constant_buffer = NULL;*/
}
...
void water_directx_12_render(struct WaterCommon* water_common, struct GBufferCommon* gbuffer_common) {
  ...
  ID3D12DescriptorHeap* heaps[] = {water_common->wave_texture->texture_common.texture_manager_common->texture_manager_directx12.srv_heap, water_common->shader->shader_common.shader_directx12.sampler_heap};
  cmd->lpVtbl->SetDescriptorHeaps(cmd, _countof(heaps), heaps);

  cmd->lpVtbl->SetGraphicsRootDescriptorTable(cmd, 2, water_common->wave_texture->texture_common.texture_directx12.gpu_heap_handle);
  cmd->lpVtbl->SetGraphicsRootDescriptorTable(cmd, 3, water_common->shader->shader_common.shader_directx12.sampler_handle_gpu);
...
void water_directx_12_update_uniforms(struct WaterCommon* water_common, struct APICommon* api_common, struct GBufferCommon* gbuffer_common, uint32_t width, uint32_t height) {
  /*struct SpriteUniformBufferObject vubo = {0};
  ...
  hr = water_common->water_directx12.vertex_constant_buffer->lpVtbl->Map(water_common->water_directx12.vertex_constant_buffer, 0, NULL, &data);
  if (SUCCEEDED(hr)) memcpy(data, &vubo, sizeof(vubo)), water_common->water_directx12.vertex_constant_buffer->lpVtbl->Unmap(water_common->water_directx12.vertex_constant_buffer, 0, NULL);

  struct WaterFragmentUniformBufferObject fubo = {0};
  fubo.time = water_common->time;
  fubo.original_width = 640.0f;
  fubo.original_height = 548.0f;
  fubo.current_width = (float)gbuffer_common->width;
  fubo.current_height = (float)gbuffer_common->height;
  ...
  if (SUCCEEDED(hr)) memcpy(data, &fubo, sizeof(fubo)), water_common->water_directx12.fragment_constant_buffer->lpVtbl->Unmap(water_common->water_directx12.fragment_constant_buffer, 0, NULL);*/
}
