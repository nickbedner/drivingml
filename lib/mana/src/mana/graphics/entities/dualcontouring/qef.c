...
static void calc_symmetric_givens_coefficients(const float a_pp, const float a_pq, const float a_qq, float *c, float *s) {
  ...
  const float stt = sqrtf(1.0f + tau * tau);
...
float qef_solver_solve(struct QefSolver *qef_solver, vec3 *outx, const float svd_tol, const int svd_sweeps, const float pinv_tol) {
  ...
  qef_solver->mass_point = vec3_scale(qef_solver->mass_point, 1.0f / (float)qef_solver->data.num_points);
  qef_solver_set_ata(qef_solver);
  qef_solver_set_atb(qef_solver);
  ...
  float fnorm_vtav = sqrtf((vtav.vecs[0].data[0] * vtav.vecs[0].data[0]) + (vtav.vecs[0].data[0] * vtav.vecs[0].data[0]) + (vtav.vecs[0].data[0] * vtav.vecs[0].data[0]) + (v.vecs[0].data[0] * v.vecs[0].data[0]) + (v.vecs[1].data[0] * v.vecs[1].data[0]) + (v.vecs[1].data[0] * v.vecs[1].data[0]) + (v.vecs[0].data[0] * v.vecs[0].data[0]) + (v.vecs[1].data[0] * v.vecs[1].data[0]) + (v.vecs[2].data[0] * v.vecs[2].data[0]));
