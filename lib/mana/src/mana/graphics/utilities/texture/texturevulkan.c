...
uint8_t texture_vulkan_init(struct TextureCommon* texture_common,
  ...
  if (layer_count == 1 && texture_common->texture_settings.mip_type == MIP_CUSTOM) {
    ...
    for (uint32_t level = 1; level < mip_count; level++) {
      ...
      VkDeviceSize sz = (VkDeviceSize)wl * (VkDeviceSize)hl *
                        (VkDeviceSize)texture_common->channels *
                        (VkDeviceSize)bytes_per_channel_local;
  ...
  if (layer_count > 1) {
    image_size = (VkDeviceSize)texture_common->width *
                 (VkDeviceSize)texture_common->height *
                 (VkDeviceSize)channels *
                 (VkDeviceSize)bytes_per_channel *
                 (VkDeviceSize)layer_count;
  } else if (texture_settings.mip_type == MIP_CUSTOM) {
  ...
  uint8_t buffer_result = vulkan_graphics_utils_create_buffer(
      api_common->vulkan_api.device,
      api_common->vulkan_api.physical_device,
      image_size,
  ...
  if (buffer_result != VULKAN_API_SUCCESS || staging_buffer == VK_NULL_HANDLE || staging_buffer_memory == VK_NULL_HANDLE) {
    log_message(LOG_SEVERITY_ERROR,
                "Failed to create staging buffer (image_size=%llu, width=%u, height=%u, layers=%u, channels=%u, bytes_per_channel=%u)\n",
  ...
  vulkan_graphics_utils_create_image(
      api_common->vulkan_api.device,
      api_common->vulkan_api.physical_device,
      texture_common->width,
      texture_common->height,
      ...
      &(texture_common->texture_vulkan.texture_image_memory));

  vulkan_graphics_utils_transition_image_layout(
      api_common->vulkan_api.device,
      api_common->vulkan_api.graphics_queue,
  ...
  if (layer_count > 1) {
    VkDeviceSize frame_size =
        (VkDeviceSize)texture_common->width *
        (VkDeviceSize)texture_common->height *
        (VkDeviceSize)channels *
        (VkDeviceSize)bytes_per_channel;
    ...
    vulkan_graphics_utils_end_single_time_commands(
        api_common->vulkan_api.device,
        api_common->vulkan_api.graphics_queue,
        api_common->vulkan_api.command_pool,
        cmd);
    ...
    vulkan_graphics_utils_copy_buffer_to_image(
        api_common->vulkan_api.device,
        api_common->vulkan_api.graphics_queue,
        api_common->vulkan_api.command_pool,
        &staging_buffer,
    ...
    if (texture_settings.mip_type == MIP_GENERATE) {
      vulkan_graphics_utils_generate_mipmaps(
          api_common->vulkan_api.device,
          api_common->vulkan_api.physical_device,
          api_common->vulkan_api.graphics_queue,
          api_common->vulkan_api.command_pool,
  ...
  vulkan_graphics_utils_create_image_view(
      api_common->vulkan_api.device,
      texture_common->texture_vulkan.texture_image,
      format,
      ...
      mip_levels,
      layer_count,
      &(texture_common->texture_vulkan.texture_image_view));
  ...
  vulkan_graphics_utils_create_sampler(
      ...
      (struct SamplerSettings){
          ...
          .address_mode = mode,
          .anisotropy_enable = anisotropy_enable_vulkan,
