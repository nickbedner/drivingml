...
static inline uint_fast8_t post_process_vulkan_init_common(struct PostProcessCommon *post_process_common, struct APICommon *api_common, struct SwapChainCommon *swap_chain_common) {
  ...
  for (uint_fast8_t ping_pong_target = 0; ping_pong_target < POST_PROCESS_PING_PONG; ping_pong_target++) {
    ...
    vulkan_graphics_utils_create_image_view(api_common->vulkan_api.device, post_process_common->post_process_vulkan.color_images[ping_pong_target], VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, 1, &(post_process_common->post_process_vulkan.color_image_views[ping_pong_target]));
