cbuffer SpriteUniformBufferObject : register(b0)
{
  ...
  float4x4 view;
  float4x4 proj;
  float3 frame_pos_xy_direction_z;
}
...
GBufferOutput PS_main(PixelInput input) : SV_Target
{
  ...
  if(frame_pos_xy_direction_z.z > 0) 
    output.albedo = tex_sampler1.Sample(sampler_state1, input.tex_coord + frame_pos_xy_direction_z.xy);
else 
    output.albedo = tex_sampler1.Sample(sampler_state1, float2(frame_pos_xy_direction_z.x - input.tex_coord.x, input.tex_coord.y + frame_pos_xy_direction_z.y));
