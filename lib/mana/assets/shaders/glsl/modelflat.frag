...
layout(binding = 2) uniform PBRPallete {
  float diffuse_pallete[COLOR_DIVS];
  float metallic_pallete[PBR_DIVS];
  float roughness_pallete[PBR_DIVS];
} pbr;

layout(location = 0) in vec2 in_tex_coord;
layout(location = 1) in vec3 in_normal;
layout(location = 2) in vec3 in_color;
layout(location = 3) in vec3 in_color_frag_pos;
layout(location = 4) in vec3 in_metallic_frag_pos;
layout(location = 5) in vec3 in_roughness_frag_pos;
